...
export const postInventory = async (req, res) => {
  const { warehouse_id, item_name, description, category, status, quantity } =
    req.body;
  console.log(warehouse_id, item_name, description, category, status, quantity);
  if (
    !warehouse_id ||
    !item_name ||
    !description ||
  ...
  try {
    const [warehouseResults] = await connection.query(
    ...
    if (warehouseResults.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid id Warehouse does not exist." });
    }
